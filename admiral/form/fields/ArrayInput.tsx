...
export const ArrayInput: InputComponentWithName<(props: ArrayInputProps) => React.JSX.Element> =
    function ArrayInput({
        ...
        const handleAdd = (_e: React.MouseEvent<HTMLButtonElement>) => {
        ...
        const renderChildrenAutoFocus = useCallback(
            (children: React.ReactNode, isNewCreatedFormItem: boolean) => {
                ...
                if (!isNewCreatedFormItem) return children

                let foundFirstInput = false

                return React.Children.map(children, (childrenItem, _index) => {
            ...
            <Form.Item label={label} columnSpan={2} labelAs="div" required={required}>
                <ol className={styles.arrayInput}>
                    {forms.map((form, idx) => {
                        ...
                        const key = idx
                        const isLastItem = idx === forms.length - 1
                        const shouldAutoFocus = justAddedRef.current && isLastItem
